...
nearby (KDTree getters _ kdt) r ns =
        ...
        else error "Search dimensions did not match KD-Tree dimensions."
    where
        dims = length ns
        fns = zip getters ns
